  ...
  create(context) {
    ...
    return {
      CallExpression(node) {
        ...
        // Matcher-aware reason: state WHY this specific assertion can never fail,
        // instead of a generic "always exists" (wrong for .to.be.ok / .to.be.an).
        let reason = "always exists";
        if (!negated && terminal === "ok") reason = "is always truthy";
        else if (!negated && (terminal === "a" || terminal === "an")) reason = "is always an object";
        ...
        context.report({
          ...
          // Autofix only the high-confidence inline cy-query case, AND only when the
          // assertion is a standalone statement — rewriting `expect(...)` used as a
          // value (e.g. `const r = expect(cy.get()).to.exist`) would change what the
          // expression evaluates to. Heuristic identifier matches are report-only.
          fix(fixer) {
